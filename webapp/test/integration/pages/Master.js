		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("TicketId") === oObject.TicketId;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].TicketId;
									} else {
										sCurrentId = aItemsNotInTheList[0].TicketId;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("TicketId"),
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "TICKETOVERDUESet (0)"
						}),
						success: function() {
							Opa5.assert.ok(true, "The list header displays 'TICKETOVERDUESet (0)'");
						},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all TICKETOVERDUESet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function(oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "TICKETOVERDUESet (" + iExpectedLength + ")"
								}),
								success: function() {
